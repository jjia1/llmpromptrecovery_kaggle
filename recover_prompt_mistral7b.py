...
def is_output_valid(output):
    invalid_sections = ["cannot answer this", "not able to provide an excerpt", "offensive", "discriminate"]
...
# TODO: Testing this
train_data_df = train_data_df.map(lambda samples: tokenizer(samples["text"], truncation=True, padding='max_length', max_length=512), batched=True)
test_data_df = test_data_df.map(lambda samples: tokenizer(samples["text"], truncation=True, padding='max_length', max_length=512), batched=True)
...
model = AutoModelForCausalLM.from_pretrained('mistralai/Mistral-7B-Instruct-v0.2', quantization_config=bnb_config, torch_dtype=torch.float16, device_map="auto")
